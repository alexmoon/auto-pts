#
# auto-pts - The Bluetooth PTS Automation Framework
#
# Copyright (c) 2025, Tactile Engineering.
#
# This program is free software; you can redistribute it and/or modify it
# under the terms and conditions of the GNU General Public License,
# version 2, as published by the Free Software Foundation.
#
# This program is distributed in the hope it will be useful, but WITHOUT
# ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or
# FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for
# more details.
#

"""SM test cases"""

from autopts.client import get_unique_name
from autopts.ptsprojects.stack import get_stack
from autopts.ptsprojects.testcase import TestFunc
from autopts.ptsprojects.trouble.sm_wid import sm_wid_hdl
from autopts.ptsprojects.trouble.ttestcase import TTestCase
from autopts.pybtp import btp
from autopts.pybtp.types import Addr, IOCap


def set_pixits(ptses):
    """Setup SM profile PIXITS for workspace. Those values are used for test
    case if not updated within test case.

    PIXITS always should be updated accordingly to project and newest version of
    PTS.

    ptses -- list of PyPTS instances"""

    pts = ptses[0]

    pts.set_pixit("SM", "TSPX_bd_addr_iut", "DEADBEEFDEAD")
    pts.set_pixit("SM", "TSPX_iut_device_name_in_adv_packet_for_random_address", "")
    pts.set_pixit("SM", "TSPX_time_guard", "180000")
    pts.set_pixit("SM", "TSPX_use_implicit_send", "TRUE")
    pts.set_pixit("SM", "TSPX_new_key_failed_count", "0")
    pts.set_pixit("SM", "TSPX_Bonding_Flags", "00")
    pts.set_pixit("SM", "TSPX_ATTR_HANDLE", "0000")
    pts.set_pixit("SM", "TSPX_ATTR_VALUE", "0000000000000000")
    pts.set_pixit("SM", "TSPX_Min_Encryption_Key_Length", "07")
    pts.set_pixit("SM", "TSPX_OOB_Data", "0000000000000000FE12036E5A889F4D")
    pts.set_pixit("SM", "TSPX_tester_role_optional", "L2CAP_ROLE_INITIATOR")


def test_cases(ptses):
    """Returns a list of SM test cases
    ptses -- list of PyPTS instances"""

    pts = ptses[0]

    pts_bd_addr = pts.q_bd_addr

    stack = get_stack()
    iut_device_name = get_unique_name(pts)
    stack.gap_init(name=iut_device_name)

    pre_conditions = [TestFunc(btp.core_reg_svc_gap),
                      TestFunc(stack.gap_init, iut_device_name),
                      TestFunc(btp.gap_read_ctrl_info),
                      TestFunc(lambda: pts.update_pixit_param(
                          "SM", "TSPX_bd_addr_iut",
                          stack.gap.iut_addr_get_str())),
                      TestFunc(lambda: pts.update_pixit_param(
                          "SM", "TSPX_iut_device_name_in_adv_packet_for_random_address",
                          iut_device_name)),
                      TestFunc(lambda: pts.update_pixit_param(
                          "SM", "TSPX_Bonding_Flags", "01"
                          if stack.gap.current_settings_get('Bondable')
                          else "00")),
                      # FIXME Find better place to store PTS bdaddr
                      TestFunc(btp.set_pts_addr, pts_bd_addr, Addr.le_public)]

    pre_conditions_privacy = [
        TestFunc(btp.core_reg_svc_gap),
        TestFunc(stack.gap_init, iut_device_name),
        TestFunc(btp.gap_read_ctrl_info),
        TestFunc(btp.gap_set_privacy_on),
        TestFunc(lambda: pts.update_pixit_param(
            "SM", "TSPX_bd_addr_iut",
            stack.gap.iut_addr_get_str())),
        TestFunc(lambda: pts.update_pixit_param(
            "SM", "TSPX_iut_device_name_in_adv_packet_for_random_address",
            iut_device_name)),
        TestFunc(lambda: pts.update_pixit_param(
            "SM", "TSPX_Bonding_Flags", "01"
            if stack.gap.current_settings_get('Bondable')
            else "00")),
        TestFunc(btp.set_pts_addr, pts_bd_addr, Addr.le_public),
    ]

    custom_test_cases = [
        # Privacy test cases (matches Zephyr privacy.conf)
        TTestCase("SM", "SM/CEN/KDU/BV-05-C",
                  pre_conditions_privacy,
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/CEN/KDU/BV-10-C",
                  pre_conditions_privacy,
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/CEN/KDU/BV-11-C",
                  pre_conditions_privacy,
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/PER/KDU/BV-02-C",
                  pre_conditions_privacy,
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/PER/KDU/BV-08-C",
                  pre_conditions_privacy,
                  generic_wid_hdl=sm_wid_hdl),

        # Privacy + no MITM (matches Zephyr sc_m1l2.conf)
        TTestCase("SM", "SM/CEN/KDU/BI-02-C",
                  pre_conditions_privacy,
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/CEN/KDU/BI-03-C",
                  pre_conditions_privacy,
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/PER/KDU/BI-02-C",
                  pre_conditions_privacy,
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/PER/KDU/BI-03-C",
                  pre_conditions_privacy,
                  generic_wid_hdl=sm_wid_hdl),

        TTestCase("SM", "SM/CEN/PKE/BI-01-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.keyboard_only)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/CEN/PKE/BI-02-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.display_only)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/CEN/PKE/BV-01-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.keyboard_display),
                   TestFunc(btp.gap_set_bondable_off)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/CEN/PKE/BV-04-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.keyboard_only)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/PER/PKE/BI-03-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.display_only)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/PER/PKE/BV-02-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.keyboard_only)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/PER/PKE/BV-05-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.keyboard_display)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/CEN/SCJW/BV-01-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_bondable_off)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/CEN/SCJW/BV-04-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_bondable_off)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/CEN/SCPK/BV-01-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.keyboard_only),
                   TestFunc(btp.gap_set_bondable_off),
                   ],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/CEN/SCPK/BV-04-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.keyboard_display),
                   TestFunc(btp.gap_set_bondable_off)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/CEN/SCPK/BI-01-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.display_yesno),
                   TestFunc(btp.gap_set_bondable_off), ],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/CEN/SCPK/BI-02-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.display_only),
                   TestFunc(btp.gap_set_bondable_on),
                   ],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/PER/SCJW/BV-02-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_bondable_off)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/PER/SCJW/BV-03-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_bondable_off)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/CEN/OOB/BV-01-C",
                  pre_conditions +
                  [TestFunc(btp.gap_oob_legacy_set_data, stack.gap.oob_legacy)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/CEN/OOB/BV-03-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.keyboard_display),
                   TestFunc(btp.gap_oob_legacy_set_data, stack.gap.oob_legacy)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/CEN/OOB/BV-09-C",
                  pre_conditions +
                  [TestFunc(btp.gap_oob_legacy_set_data, stack.gap.oob_legacy)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/CEN/OOB/BI-01-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.display_only),
                   TestFunc(btp.gap_oob_legacy_set_data, stack.gap.oob_legacy)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/PER/OOB/BV-02-C",
                  pre_conditions +
                  [TestFunc(btp.gap_oob_legacy_set_data, stack.gap.oob_legacy)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/PER/OOB/BV-04-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.keyboard_display),
                   TestFunc(btp.gap_oob_legacy_set_data, stack.gap.oob_legacy)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/PER/OOB/BV-10-C",
                  pre_conditions +
                  [TestFunc(btp.gap_oob_legacy_set_data, stack.gap.oob_legacy)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/PER/OOB/BI-02-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.display_only),
                   TestFunc(btp.gap_oob_legacy_set_data, stack.gap.oob_legacy)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/CEN/SIGN/BV-01-C",
                  pre_conditions +
                  [TestFunc(btp.core_reg_svc_gatt)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/PER/SCPK/BV-02-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.keyboard_display)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/PER/SCPK/BV-03-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.display_only)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/PER/SCPK/BI-03-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.display_yesno)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/PER/SCPK/BI-04-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.keyboard_only)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/PER/SIP/BV-01-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.keyboard_display)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/CEN/SIP/BV-02-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.keyboard_display)],
                  generic_wid_hdl=sm_wid_hdl),
        TTestCase("SM", "SM/PER/SIE/BV-01-C",
                  pre_conditions +
                  [TestFunc(btp.gap_set_io_cap, IOCap.no_input_output), ],
                  generic_wid_hdl=sm_wid_hdl),
    ]

    test_case_name_list = pts.get_test_case_list('SM')
    tc_list = []

    for tc_name in test_case_name_list:
        instance = TTestCase('SM', tc_name,
                             pre_conditions +
                             [TestFunc(btp.gap_set_io_cap, IOCap.display_only)],
                             generic_wid_hdl=sm_wid_hdl)

        for custom_tc in custom_test_cases:
            if tc_name == custom_tc.name:
                instance = custom_tc
                break

        tc_list.append(instance)

    return tc_list
